...
function handleDesktopNavigation() {
  document.addEventListener("click", (e) => {
    ...
    t && !o
      ...
      : (n || (activeDropdown && !o)) && closeAllDropdowns();
  });
}
...
function lastWordColor() {
  const element = document.querySelector(".last-word-color");
  if (!element) return;
  const words = element.innerHTML.split(" ");
  const lastWord = words.pop();
  element.innerHTML =
    words.join(" ") + `<span class="text-secondary"> ${lastWord}</span>`;
}
