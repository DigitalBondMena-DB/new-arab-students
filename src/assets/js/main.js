const Animations = (() => {
  ...
  n = t => {
    const n = t.dataset.animationDuration || e.duration,
      o = t.dataset.animationDelay || "0s";
    requestAnimationFrame(() => {
      t.style.transition = `transform ${n} ${e.timing}, opacity ${n} ${e.timing}`,
      t.style.transitionDelay = o,
      t.style.opacity = "1",
  ...
  return {
    init: (n = {}) => {
      try {
        const o = { ...e, ...n },
          a = document.querySelectorAll("[data-animate]");
        if (0 === a.length) return;
        const s = new IntersectionObserver(t, { threshold: o.threshold, rootMargin: o.rootMargin });
        a.forEach(t => {
          ((t) => {
            t.dataset.animationDirection;
            const n = t.dataset.animationDistance || e.distance;
            t.dataset.animationDuration,
            t.dataset.animationDelay,
            t.style.opacity = "0",
            t.style.transition = "none",
            t.style.willChange = "transform, opacity";
            let o = "0", a = "0";
            const s = "rtl" === document.dir || "rtl" === document.documentElement.dir;
            switch (t.dataset.animationDirection || "start") {
              case "start": o = s ? n : `-${n}`; break;
              case "end": o = s ? `-${n}` : n; break;
              case "top": a = `-${n}`; break;
              case "bottom": a = n
            }
            t.style.transform = `translate3d(${o}, ${a}, 0)`
          })(t),
          s.observe(t)
        })
      } catch (e) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  handleSocialSidebar(),
  initSidebarThemeObserver(),
  handleGlobalEvents()
});

let activeDropdown = null;

function initSidebarThemeObserver() {
  const e = document.getElementById("social-bar");
  ...
  const t = new IntersectionObserver(t => {
    t.forEach(t => {
      if (!t.isIntersecting) {
        if (e) {
          e.classList.add("social-bar--light");
          e.classList.remove("text-natural-25");
        }
        if (registerBtn) {
        ...
        if (e) {
          e.classList.remove("social-bar--light");
          e.classList.add("text-natural-25");
        }
        if (registerBtn) {
          const p = registerBtn.querySelector("p");
          if (p) {
    ...
    t.forEach(e => {
      Array.from(e.children).forEach(e => {
        ...
        if (a) {
          ...
          t.className = "flex items-center justify-between w-full hover:text-primary transition-colors",
          ...
          s.className = "hidden flex-col gap-3 pr-4 mt-3 text-lg text-natural-500",
          ...
          e.className = "hover:text-primary transition-colors border-b border-natural-300 pb-2",
          n.appendChild(e)
        }
      })
    });
    const o = Array.from(e.querySelectorAll("a.border-secondary, a[href*='contact']"));
    e.innerHTML = "",
    e.appendChild(n),
    o.forEach(t => {
...
function handleDesktopNavigation() {
  document.addEventListener("click", e => {
    const t = e.target.closest("[data-dropdown-trigger]"),
      ...
      o = e.target.closest(".dropdown-menu");
    t && !o ? (e.preventDefault(), e.stopPropagation(), toggleDropdown(t)) : (n || activeDropdown && !o) && closeAllDropdowns()
  })
}
...
function toggleDropdown(e) {
  ...
  t && (t.classList.contains("visible") ? closeDropdown(t) : openDropdown(t, n))
}
...
function closeAllDropdowns() {
  document.querySelectorAll(".dropdown-menu.visible").forEach(e => closeDropdown(e))
}
...
function setSidenavState(e) {
  ...
  document.body.style.overflow = "hidden") : (o.classList.remove("opacity-100"),
...
function toggleMobileDropdown(e) {
  ...
  t.classList.contains("hidden") ? (t.classList.remove("hidden"), t.classList.add("flex"), n.classList.add("rotate-180")) : (t.classList.add("hidden"), t.classList.remove("flex"), n.classList.remove("rotate-180"))
}
...
function handleSocialSidebar() {
  ...
  if (!e || !t) return;
  const n = "rtl" === document.dir ? "-translate-x-[100px]" : "translate-x-[100px]";
